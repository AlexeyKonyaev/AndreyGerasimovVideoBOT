...
def main(message):
    ...
    if msg == '/start':
        ...
        send(id, 'ПРИМЕР РАССЧЁТА ЦЕНЫ: ✔300 руб - просто соединить видео в одно с обычными плавными переходами. ✔Ещё +50 руб - наложить фоновую музыку с затуханиями и возрастаниями в начале, конце и в перерывах (если видео состоит из нескольких глав и частей - пример в группе) ✔Ещё +25 руб - это сделать разные переходы. ✔Ещё + 75 руб - это добавить 1 всплывающую надпись в какой-то 1 момент видео.')
    elif msg == 'Обо Мне':
        ...
        bot.send_message(message.from_user.id, 'Если вы крутой монтаж, то напишите сколько частей видео нужно соединить?')
...
def reg_contact(message):
    ...
    bot.send_message(message.from_user.id, 'Заявка отправлена успено!')
